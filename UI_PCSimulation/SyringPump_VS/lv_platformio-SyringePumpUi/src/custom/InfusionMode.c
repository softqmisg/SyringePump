...
void loadDefaultInfusionModeValue(void)
{
    for(modeid_t modes=modeVol;modes<=modeLinear;modes++)
    {
        for(uint8_t units=0;units<8;units++)
        {
          ...
          DefaultModes[modes][units].TotalVolume10=(uint16_t)lv_rand(0,18000);
          DefaultModes[modes][units].InfusionRate10=(uint16_t)lv_rand(0,990);
          DefaultModes[modes][units].TotalTime=(time_t)lv_rand(0,36000);
          DefaultModes[modes][units].BodyWeight=(int16_t)lv_rand(0,300);
        }
    }
