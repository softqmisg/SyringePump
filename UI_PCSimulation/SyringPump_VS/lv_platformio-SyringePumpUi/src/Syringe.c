...
void loadDefaultSyringesValue(void)
{
    ...
    for(uint8_t mfg=0;mfg<50;mfg++)
    {
        for(uint8_t sy=0;sy<20;sy++)
        {
            ...
            DefaultSyrings[mfg][sy].InnerDia10=(uint16_t)randi(0,1000);
            DefaultSyrings[mfg][sy].OuterDia10=(uint16_t)randi(0,1000);
            DefaultSyrings[mfg][sy].BarrelLen10=(uint16_t)randi(0,1000);
            DefaultSyrings[mfg][sy].PlungerLen10=(uint16_t)randi(0,1000);
